...
try {
    ...
    if (!empty($nifApiKey) && preg_match('/^[0-9]{9}$/', $restaurantCode)) {
        try {
            ...
            // Se a API falhar (timeout/500/exception), ignoramos e seguimos com o registo para não bloquear os utilizadores.
            error_log('NIF.pt validation failed: ' . $e->getMessage());
        }
    }
    // -------------------------
